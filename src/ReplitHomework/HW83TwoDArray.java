...
public class HW83TwoDArray {
    ...
    public static void main(String[] args) {
        ...
        int[] sums=new int[a.length];  //create an array to store sums

        for (int i = 0; i < a.length; i++) {
            int sum=0;
            for (int j = 0; j < a[i].length; j++) {
                sum+=a[i][j];  // add up the elements in the current row
            }
            sums[i]=sum;  // store the sum in the array
        }
        // print out the sums
